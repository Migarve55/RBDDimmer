...
This is a fork of this [library](https://github.com/RobotDynOfficial/RBDDimmer)

I created it because it was using a decretaed library for the esp32 family of boards, and it did not compile.
...
1. Function dimmerLamp - this function initializes the number of operating pin and is defined by the user
   ```cpp
   // dimmer output DIM/PSM is initialized on the pin 4 for the bords WITHOUT changable ZERO-CROSS input pin (AVR, Arduino M0/Zero)
   a.dimmerLamp dimmer(4);
   // dimmer output DIM/PSM is initialized on the pin 4 and zero-cross initialized on pin 2. Only for boards whith changable zero-cross(ESP32, ESP8266, Arduino Due)
   b.dimmerLamp dimmer(4, 2); 
   ```
...
2. Function begin port initialization, timer and external interrupt from zero-cross
 ...
 Parameter 1: dimmer working modes consist of two choices - NORMAL_MODE and TOGGLE_MODE
  - NORMAL_MODE: to make dimmer work in defined value from 0 to 100 (%) (integer)
            Example of this mode located in \RBDdimmer\examples\SimpleDimmer
  - TOGGLE_MODE: smooth change of dimming value up or down in a defined range. This solutions implies change of dimming values by means of hardware timer, without using the cycle code.
...
15. Function changeState changes dimmer state to the opposite one
```cpp
dimmer.setState(ON);
delay(100);
dimmer.changeState; delay(100);
```
  
16. Function toggleSettings smooth change of dimming value up or down in a defined range
